...
describe('runtime DOM validation contract', () => {
  it('checks native and Ovlira control names without inspecting component internals', () => {
    document.body.innerHTML = '<label for="name">Workspace name</label><input id="name"><ov-input label="Region"></ov-input><ov-textarea></ov-textarea><ov-checkbox></ov-checkbox><ov-radio-group></ov-radio-group><ov-toggle></ov-toggle><ov-input></ov-input>';

    const diagnostics = validateRuntimeDocument(document, { definedTags: new Set(['ov-input', 'ov-textarea', 'ov-checkbox', 'ov-radio-group', 'ov-toggle']) });

    expect(diagnostics.map((diagnostic) => diagnostic.ruleId)).toEqual(['runtime.required-label', 'runtime.required-label', 'runtime.required-label', 'runtime.required-label', 'runtime.required-label']);
    ...
    expect(diagnostics[4]?.message).toContain('ov-input');
  });
