...
test('review theme control cycles through both default schemes and the contrasting theme', async ({ page }) => {
  await page.goto('/?recipe=page.settings');
  ...
  await expect(page.getByText('Contrasting theme', { exact: true })).toBeVisible();
  await toggle.click();
  await expect(root).toHaveAttribute('data-theme', 'light');
});
...
for (const theme of themes) {
  for (const viewport of [{ width: 1440, height: 900 }, { width: 768, height: 1024 }, { width: 375, height: 812 }]) {
    test(`${theme} fixtures do not overflow at ${viewport.width}px`, async ({ page }) => {
...
for (const theme of themes) {
  test(`${theme} narrow interactive targets remain touch-sized`, async ({ page }) => {
