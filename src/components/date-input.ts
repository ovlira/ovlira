...
/** A labelled native date field with help, error, and range constraints. */
export class OvDateInput extends LitElement {
  static properties = {
  ...
  error = '';
  helpText = '';
  private readonly inputId = `ov-date-input-${++dateInputId}`;
  ...
  render() {
    ...
    return html`
      <div class="field" part="field">
        ...
        <input id=${this.inputId} name=${this.name || nothing} type="date" .value=${this.value} min=${this.min || nothing} max=${this.max || nothing} ?required=${this.required} ?disabled=${this.disabled} aria-invalid=${this.error ? 'true' : 'false'} aria-describedby=${this.error || this.helpText ? messageId : nothing} part="input" @input=${this.handleInput} @change=${this.handleChange}>
...
customElements.define('ov-date-input', OvDateInput);

declare global {
  interface HTMLElementTagNameMap {
    'ov-date-input': OvDateInput;
  }
}
