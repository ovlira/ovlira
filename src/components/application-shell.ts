...
export class OvApplicationShell extends LitElement {
  ...
  static styles = css`
    ...
    nav::slotted(*) { color: rgb(255 253 246 / 0.7); font: 650 var(--ov-text-sm, 0.84rem) / 1.2 var(--ov-font-mono, monospace); padding: 0.7rem 0.8rem; text-decoration: none; }
    nav::slotted(*:hover), nav::slotted([aria-current="page"]) { background: var(--ov-color-accent, #c7f36b); color: var(--ov-color-ink, #1d211d); }
    .content { min-width: 0; }
  ...
  render() {
    return html`<div class="shell" part="shell">
      <aside class="rail" part="rail"><div class="brand" part="brand"><slot name="brand"></slot></div><nav part="nav" aria-label=${this.navLabel}><slot name="nav"></slot></nav></aside>
