...
export class OvToggle extends LitElement {
  ...
  static styles = css`
    ...
    .toggle-control { background-color: var(--ov-border, var(--ov-color-line, rgb(0 0 0 / 0.10))); border: 1px solid var(--ov-border, var(--ov-color-line, rgb(0 0 0 / 0.10))); border-radius: 999px; block-size: 1.25rem; display: inline-block; inline-size: 2.25rem; position: relative; transition: background-color var(--ov-motion-fast, 150ms) ease, border-color var(--ov-motion-fast, 150ms) ease; }
    .toggle-control:has(input:checked) { background-color: var(--ov-good, var(--ov-color-success, #15743a)); border-color: var(--ov-good, var(--ov-color-success, #15743a)); }
    .toggle-control:has(input[aria-invalid="true"]) { border-color: var(--ov-bad, var(--ov-color-danger, #b42318)); }
    .toggle-control:has(input:focus-visible) { outline: 2px solid var(--ov-focus, var(--ov-color-accent-strong, #525252)); outline-offset: 2px; }
    .thumb { background: var(--ov-bg, var(--ov-color-canvas, #fff)); border: 1px solid var(--ov-border, var(--ov-color-line, rgb(0 0 0 / 0.10))); border-radius: 50%; block-size: 0.9rem; display: block; inline-size: 0.9rem; inset-block-start: 0.1rem; inset-inline-start: 0.1rem; position: absolute; transition: transform var(--ov-motion-fast, 150ms) ease; }
    input {
      appearance: none;
    ...
    input:checked + .thumb { transform: translateX(1rem); }
    ...
    return html`
      <div class="field" part="field">
        <label part="label" for=${this.toggleId}>
          ...
          <span class="toggle-control" part="toggle">
            <input
              id=${this.toggleId}
              type="checkbox"
              role="switch"
