...
function settingsMarkup(): string {
  ...
  <div class="settings-page">
    ...
    <div class="state-stack">
      ...
      <ov-alert data-ovlira-state="error" tone="danger" heading="Could not save">Check the highlighted fields and try again.</ov-alert>
...
function genericRecipeMarkup(recipe: RecipeDescriptor): string {
  const states = recipe.requiredStates.map((state) => `<div data-ovlira-state="${state}" hidden>${state} state</div>`).join('');
  if (recipe.id === 'shell.application') return `<ov-application-shell><a slot="brand" class="brand-mark" href="/">OVLIRA</a><a slot="nav" href="/" aria-current="page">Overview</a><div class="demo"><ov-page-header title="Application shell" description="A persistent frame for a small local application."></ov-page-header></div></ov-application-shell>`;
  if (recipe.id === 'state.empty') return `<main class="demo"><p class="kicker">OVLIRA / RECIPE</p>${states}<ov-empty-state data-ovlira-state="empty" title="Nothing here yet" description="Create the first record to begin."><ov-button slot="action" variant="primary">Create record</ov-button></ov-empty-state></main>`;
  if (recipe.id === 'page.detail') return `<main class="demo"><p class="kicker">OVLIRA / RECIPE</p>${states}<ov-page-header title="Record detail" description="A focused view of one record."></ov-page-header><ov-card><span slot="header">Record summary <ov-badge tone="success">Active</ov-badge></span><p>Group the fields needed to understand this record before taking action.</p><span slot="footer"><ov-button variant="primary">Edit record</ov-button></span></ov-card></main>`;
  const first = recipe.id.includes('search') ? '<ov-input label="Search records" placeholder="Name, email, or ID"></ov-input><ov-button variant="primary">Search</ov-button>' : `<ov-page-header title="${recipe.title}" description="${recipe.description}"></ov-page-header>`;
  return `<main class="demo"><p class="kicker">OVLIRA / RECIPE</p>${states}${first}<ov-alert tone="danger">An example error state.</ov-alert><ov-empty-state title="Nothing here yet" description="The collection will appear here when it has records."></ov-empty-state></main>`;
}
