...
/** A native dialog for interruptive decisions with explicit open and modal semantics. */
export class OvDialog extends LitElement {
  static properties = {
  ...
  private readonly instanceId = `ov-dialog-${++dialogId}`;
  ...
  static styles = css`
    ...
    :host([modal]) dialog { inset: 0; position: fixed; }
    dialog::backdrop { background: color-mix(in srgb, var(--ov-text, var(--ov-color-ink, #171717)) 30%, transparent); }
    .surface {
      background: var(--ov-surface-raised, var(--ov-color-surface-raised, #ffffff));
      border: 1px solid var(--ov-border, var(--ov-color-line, rgb(0 0 0 / 0.10)));
      border-radius: var(--ov-radius-lg, 8px);
      box-shadow: var(--ov-shadow-md, 0 10px 30px rgb(0 0 0 / 0.12));
      max-block-size: calc(100vh - 2rem);
      overflow: auto;
      padding: var(--ov-space-6, 1.5rem);
    }
    ...
    .body { font: 400 var(--ov-text-md, 0.875rem) / var(--ov-line-body, 1.55) var(--ov-font-sans, sans-serif); margin-block-start: var(--ov-space-6, 1.5rem); }
    .body::slotted(*) { margin-block: 0; }
    footer { align-items: center; display: flex; flex-wrap: wrap; gap: var(--ov-space-2, 0.5rem); justify-content: flex-end; margin-block-start: var(--ov-space-6, 1.5rem); }
    footer[hidden] { display: none; }
    .close {
      appearance: none;
      background: transparent;
      border: 0;
      border-radius: var(--ov-radius-sm, 5px);
      color: var(--ov-muted, var(--ov-color-muted, #6f6f6f));
      cursor: pointer;
      font: 500 var(--ov-text-sm, 0.82rem) / 1 var(--ov-font-sans, sans-serif);
    ...
    @media (max-width: 47.5rem) {
      dialog { max-block-size: calc(100vh - 1rem); max-inline-size: calc(100vw - 1rem); }
      .surface { max-block-size: calc(100vh - 1rem); padding: var(--ov-space-4, 1rem); }
    }
    ...
    return html`
      ...
      >
        <div class="surface" part="surface">
          <header part="header">
            <div>
              <h2 id=${titleId} part="heading">${this.heading}</h2>
              ${this.description ? html`<div id=${descriptionId} class="description" part="description">${this.description}</div>` : nothing}
            </div>
  ...
  private syncNativeDialog() {
    ...
    // A mode switch is an internal reconfiguration. Toggling the native open
    // property avoids firing a close event that would also clear the public
    // `open` state before the new mode is applied.
  ...
  private handleNativeClose = () => {
    this.nativeMode = null;
    if (this.suppressNativeClose) return;
    if (!this.open) return;
    this.open = false;
...
customElements.define('ov-dialog', OvDialog);
